...
public:
	...
	bool CCameraDS::OpenCamera(int nCamID, bool bDisplayProperties=true, int nWidth=320, int nHeight=240, int nExpose=-6, double nfocusLen=32);
	...
	int m_nCamID;
	bool isOpened();
	int  OpenCameraVideoProperty();
	int  OpenCameraProperty(CString sName);
	HRESULT ShowFilterPropertyPages(IBaseFilter *pFilter, CString sName);
};
