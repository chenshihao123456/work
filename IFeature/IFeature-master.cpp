...
__declspec(dllexport)  int WINAPI RCN_CheckRecognise(DWORD dwImg, RCNDATA *pRcnData, int &iImgReversal, int iReserve)
{
	...
	{	
		...
		sprintf(buf_tmp, "票据名称： %s", pRcnData->szInputName);
...
DWORD WINAPI FuncCameraThread(void *lpParam)
{
	...
	if (!theApp.m_pcap->OpenCamera(iIndex, false, theApp.m_width_camera, theApp.m_height_camera))
	{
		iRet = -3;
		CloseHandle(theApp.m_hCameraThread);
		theApp.m_hCameraThread = NULL;
				...
				*/
				...
				if (p->bDigitFlag != 0)
				{
					...
					if (p->nDigitDirection != 0)
					{
						...
						cvCvtColor(pDstImg, graySrc, CV_BGR2GRAY);
					...
					if (pApp->cvtImagex)
					{
						...
						{
							GetSystemTime(&st);
							sFileName.Format("%s\\Image%04d%02d%02d_%02d%02d%02d_%03d_编号图.bmp", (LPSTR)(LPCTSTR)pApp->m_strPath, st.wYear, st.wMonth, st.wDay, st.wHour + 8, st.wMinute, st.wSecond, st.wMilliseconds);
						}
						int iArr[7];
						iArr[0] = 0;
						iArr[1] = 0;
...
int WINAPI RCN_GetCutPictureSealPointByRecognize(RCNDATA *pRcnData)
{
	...
	{	
		...
		{
			//上；
			pRcnData->iCheckX *= theApp.m_pixNumPerMm;
			pRcnData->iCheckY *= theApp.m_pixNumPerMm;
		}
		else if(pRcnData->bDirectionTitle == 1)
		{
			//下
			pRcnData->iCheckX = pRcnData->iCutWidth - (pRcnData->iCheckX)*theApp.m_pixNumPerMm;
			pRcnData->iCheckY = pRcnData->iCutHeight - (pRcnData->iCheckY)*theApp.m_pixNumPerMm;
		}
		else if(pRcnData->bDirectionTitle == 2)
		{
			...
			pRcnData->iCheckX = pos_y_mm * theApp.m_pixNumPerMm;
			pRcnData->iCheckY = pRcnData->iCutHeight -  pos_x_mm * theApp.m_pixNumPerMm;
		}
		else if(pRcnData->bDirectionTitle == 3)
		{
			...
			int pos_x_mm = pRcnData->iCheckX;
			int pos_y_mm = pRcnData->iCheckY;
			pRcnData->iCheckX = pRcnData->iCutWidth - (pos_y_mm * theApp.m_pixNumPerMm);
			pRcnData->iCheckY = pos_x_mm * theApp.m_pixNumPerMm;
		}
		pRcnData->iX = pRcnData->iCheckX;
		pRcnData->iY = pRcnData->iCheckY;
	}
...
__declspec(dllexport)  int WINAPI RCN_VerifySealCodeBarNew(char *szInVerifyCode, POINT point, int iImgReversal, int iReserve, char* sealCodeOut)
{
	...
	else if (len == 0)
	{
		...
		if (result.size() >= 14)
		{
			memcpy(sealCodeOut, result.c_str(), 14);
		}
		else
		{
...
int WINAPI recognizeSealCode(cv::Mat img, std::string &result, std::vector<cv::Rect>& rects, int ocr_type = 0)
{
	...
	*/
	...
	IplImage* pImg = &IplImage(img_Threshold);
#if 1
	...
	if (ri != 0)
	{
		...
		do {
			...
			if (strcmp(word, " ") != 0 && (abs(y2 - y1) >= h / 8) && (abs(y2 - y1) <= h / 2) && (abs(x2 - x1) <= w / 7))
			{
				/*
				if(*(BYTE*)word == 56)  //处理B有可能识别成8
				{
				icc++;
				IplImage* imgNo = cvCreateImage(cvSize(rect.width , rect.height), IPL_DEPTH_8U, 1);
				cvSetImageROI(pImg, rect);
				cvCopy(pImg, imgNo);
				cvResetImageROI(pImg);

				pData = (BYTE *)imgNo->imageData;
				istep = imgNo->widthStep/sizeof(BYTE);
				...
				icount = 0;
				for(int i=imgNo->height/2 - 1; i<imgNo->height/2+1; i++)  //中
				{
				bByte = pData[i*istep+0];
				if(bByte > 200)
				{
				icount++;
				}
				...
				icount = 0;
				for(int i=imgNo->height-1; i<imgNo->height; i++)  //下
				{
				for(int j=0; j<2; j++)
				{
				bByte = pData[i*istep+j];
				...
				*(BYTE*)word = 66; //大写B
				...
				{
				icount = 0;
				for(int i=imgNo->height/2 - 2; i<imgNo->height/2+2; i++)  //中
				{
				for(int j=0; j<2;j++)
				{
				bByte = pData[i*istep+j];
				...
				*(BYTE*)word = 66; //大写B
				...
				{
				icount = 0;
				for(int i=imgNo->height/2 - 5; i<imgNo->height/2; i++)
				{
				for(int j=imgNo->width-4; j<imgNo->width-1; j++)
				{
				bByte = pData[i*istep+j];
				if(bByte > 200)
				{
				...
				*(BYTE*)word = 54; //6
				...
				//CString ss;
				//ss.Format("E:\\work\\OCRSealx\\bin\\22\\it%02d.jpg", icc);
				//cvSaveImage((LPSTR)(LPCTSTR)ss, imgNo);
				cvReleaseImage(&imgNo);
				}
				else if(*(BYTE*)word == 66)  //处理8有可能识别成B
				{
				icc++;
				IplImage* imgNo = cvCreateImage(cvSize(rect.width , rect.height), IPL_DEPTH_8U, 1);
				cvSetImageROI(pImg, rect);
				...
				*(BYTE*)word = 56; //8
				...
				//CString ss;
				//ss.Format("E:\\work\\OCRSealx\\bin\\22\\i8%02d.jpg", icc);
				//cvSaveImage((LPSTR)(LPCTSTR)ss, imgNo);
				cvReleaseImage(&imgNo);
				}
				else if(*(BYTE*)word == 68)  //处理B有可能识别成D
				{
				icc++;
				IplImage* imgNo = cvCreateImage(cvSize(rect.width , rect.height), IPL_DEPTH_8U, 1);
				cvSetImageROI(pImg, rect);
				...
				*(BYTE*)word = 48; //0
				...
				//CString ss;
				//ss.Format("E:\\work\\OCRSealx\\bin\\22\\iB%02d.jpg", icc);
				//cvSaveImage((LPSTR)(LPCTSTR)ss, imgNo);
				cvReleaseImage(&imgNo);
				}
				else if(*(BYTE*)word == 48)  //处理C有可能识别成0
				{
				icc++;
				IplImage* imgNo = cvCreateImage(cvSize(rect.width , rect.height), IPL_DEPTH_8U, 1);
				cvSetImageROI(pImg, rect);
				...
				for(int i=imgNo->height/2 - 2; i<imgNo->height/2+2; i++)  //中
				{
				for(int j=imgNo->width-3; j<imgNo->width-1; j++)
				{
				bByte = pData[i*istep+j];
				if(bByte < 200)
				{
				...
				*(BYTE*)word = 67; //大写C
				...
				icount = 0;
				for(int i=imgNo->height-2; i<imgNo->height-1; i++)  //下
				{
				for(int j=0; j<2; j++)
				{
				bByte = pData[i*istep+j];
				...
				*(BYTE*)word = 68; //大写D
				...
				//CString ss;
				//ss.Format("E:\\work\\OCRSealx\\bin\\22\\i0%02d.jpg", icc);
				//cvSaveImage((LPSTR)(LPCTSTR)ss, imgNo);
				cvReleaseImage(&imgNo);
				}
				*/
...
__declspec(dllexport)  int WINAPI RCN_CorrectionDeviceCoordinate2(DWORD dwImg, POINT OutArraySealCenter[30], int sealType, int iReserve)
{
	...
	for(int indexCircle = 0; indexCircle<result_shape_indexs.size(); indexCircle++)
	{
		...
		center.x += cutEdgeWidth;
		center.y += cutEdgeWidth;
